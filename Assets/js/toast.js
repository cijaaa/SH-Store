...
class Toasts {
    ...
    openToast(toast) {
        ...
        toast.element.style.transition = 'transform ' + this.duration + ' ' + this.timing; // Change here
        ...
        toast.element.addEventListener('transitionend', () => {
            if (toast.element.dataset.transitionState == 'opening') {
                ...
                if (toast.props.dismissAfter) {
                    setTimeout(() => this.closeToast(toast), parseFloat(toast.props.dismissAfter) * 1000); // Modify here
                }
            }
        });
        for (let i = 0; i < this.stack.length; i++) {
            if (this.stack[i].element.dataset.transitionState == 'complete') {
                ...
                this.stack[i].element.style.transition = 'transform ' + this.duration + ' ' + this.timing + (this.dimOld ? ', opacity ' + this.duration + ' ease' : ''); // Change here
                if (this.dimOld) {
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    toasts.push({
        ...
        content: 'discord.gg/shservices',
        link: 'https://discord.gg/shservices',
        linkTarget: '_blank',
        style: 'dark',
        closeButton: true,
        duration: '1s',
    ...
    const buyButtons = document.querySelectorAll('[data-sellix-product]');

    buyButtons.forEach(function(button) {
        const productId = button.getAttribute('data-sellix-product');
        const formContainer = document.getElementById(`sellix-payment-form-${button.id.replace('buyButton_', '')}`);

        button.addEventListener('click', function() {
            const handler = SelllixIoEmbedded.createHandler({
                productId: productId,
                container: formContainer,
                apiKey: 'rCjZxZkDaTth7fZmLsArZNha8VPWf3KNIDDQOjlidl8y5qjael7MJ8qcHdPcYrYI'
            });
